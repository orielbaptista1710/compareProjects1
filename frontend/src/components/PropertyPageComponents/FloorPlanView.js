...
const FloorPlanView = ({ floorPlans = [], property = {} }) => {
  ...
  // Memoize room areas calculation
  const roomAreas = useMemo(() => {
    return (
      currentFloor.rooms?.map((room) => {
        if (!room.dimensions) return 0;
        const [w, h] = room.dimensions
          .split("x")
        ...
        <nav className="floor-selector" aria-label="Select floor">
          {floorPlans.map((floor, index) => (
            ...
            >
              ...
              {floor.type && (
                <span className="floor-type-badge">{floor.type}</span>
              )}
            </button>
          ))}
        </nav>
          ...
          <section className="floor-summary">
            ...
            {totalArea != null && (
              <div className="summary-item unit-toggle">
                <FontAwesomeIcon icon={faRulerCombined} className="summary-icon" />
            ...
            {currentFloor.rooms?.length > 0 && (
              <div className="summary-item">
                <FontAwesomeIcon icon={faBath} className="summary-icon" />
